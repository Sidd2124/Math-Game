...
import Sidd from './tablist'

import MainImag from './main'
...
class App extends Component {
  ...
  India = id => {
    ...
    if (start.id === id) {
      this.setState(prevState => ({prime: prevState.prime + 1}))
    } else {
      this.setState({prime: 0})
    }
  }
  ...
  render() {
    const {count, MainList, ActiveTabid, start, prime} = this.state
    const Filter = MainList.filter(each => each.category === ActiveTabid)
    return (
      <div className="main">
        <nav className="nav">
          <img
        ...
        <div className="layer">
          <MainImag thumbnail={start} />
          <div className="buttons">
            {tabsList.map(each => (
              <Sidd tab={each} Dhoni={this.Thala} Case={this.Pass} />
            ))}
          </div>

          <div className="buttons">
            {Filter.map(each => (
              <ImageList Chennai={each} South={this.India} className="pic" />
            ))}
          </div>
        </div>
      </div>
